...
(function () {
  ...
  function DrawCanvasRAF(t) {
    var tc = globe.tc,
      i;
    globe.NextFrame(globe.interval);
    t = t || TimeNow();
  ...
  Oproto.DrawPulsate = function (c, tag, x1, y1) {
    ...
    this.pulse = ga = globe.Smooth(1, ga);
  ...
  function globe(cid, lctr, opt) {
    ...
    for (i in globe.options) this[i] = opt && Defined(opt[i]) ? opt[i] : Defined(globe[i]) ? globe[i] : globe.options[i];
    ...
    if (~~this.scrollPause > 0) globe.scrollPause = ~~this.scrollPause;
    ...
    this.animTiming = typeof globe[this.animTiming] == "function" ? globe[this.animTiming] : globe.Smooth;
    ...
    if (!globe.started) {
      globe.NextFrame = window.requestAnimationFrame ? NextFrameRAF : NextFrameTimeout;
      globe.interval = this.interval;
      globe.NextFrame(this.interval);
      globe.started = 1;
    }
  }
  TCproto = globe.prototype;
  ...
  TCproto.Draw = function (t) {
    ...
    var cv = this.canvas,
      ...
      ch = cv.height,
      max_sc = 0,
      tdelta = ((t - this.time) * globe.interval) / 1000,
      x = cw / 2 + this.offsetX,
      y = ch / 2 + this.offsetY,
      c = this.ctxt,
      active,
  ...
  globe.Start = function (id, l, o) {
    globe.Delete(id);
    globe.tc[id] = new globe(id, l, o);
  };

  function tccall(f, id) {
    globe.tc[id] && globe.tc[id][f]();
  }
  globe.Linear = function (t, t0) {
    return t0 / t;
  };
  globe.Smooth = function (t, t0) {
    return 0.5 - cos((t0 * Math.PI) / t) / 2;
  };
  globe.Pause = function (id) {
    tccall("Pause", id);
  };
  globe.Resume = function (id) {
    tccall("Resume", id);
  };
  globe.Reload = function (id) {
    tccall("Load", id);
  };
  globe.Update = function (id) {
    tccall("Update", id);
  };
  globe.SetSpeed = function (id, speed) {
    if (IsObject(speed) && globe.tc[id] && !isNaN(speed[0]) && !isNaN(speed[1])) {
      globe.tc[id].SetSpeed(speed);
      return true;
    }
    return false;
  };
  globe.TagToFront = function (id, options) {
    if (!IsObject(options)) return false;
    options.lat = options.lng = 0;
    return globe.RotateTag(id, options);
  };
  globe.RotateTag = function (id, options) {
    if (IsObject(options) && globe.tc[id]) {
      if (isNaN(options.time)) options.time = 500;
      var tt = globe.tc[id].FindTag(options);
      if (tt) {
        globe.tc[id].RotateTag(tt, options.lat, options.lng, options.time, options.callback, options.active);
        return true;
      }
    }
    return false;
  };
  globe.Delete = function (id) {
    var i, c;
    if (handlers[id]) {
  ...
  for (i in globe.options) globe[i] = globe.options[i];
  window.globe = globe;
